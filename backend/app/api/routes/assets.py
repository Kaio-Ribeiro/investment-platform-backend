...
async def search_yahoo_asset(symbol: str):
    asset_data = await yahoo_finance.search_asset(symbol)
    return asset_data

@router.post("/from-yahoo/{symbol}", response_model=AssetSchema)
async def create_asset_from_yahoo(symbol: str, db: AsyncSession = Depends(get_db)):
...
async def read_asset(asset_id: int, db: AsyncSession = Depends(get_db)):
